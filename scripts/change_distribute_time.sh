...
if [ "$USE_HOST" = true ]; then
    ...
    USE $DATABASE;
    show tables;
    UPDATE $TABLE SET scheduled_delivery='$SCHEDULED_DELIVERY' WHERE address='$ADDRESS';
    SELECT ROW_COUNT();
    COMMIT;
EOF
else
...
# Add the curl command here
curl --location --request POST 'http://127.0.0.1:8081/v1/info/distribute_airdrops' \
